...
public class JoinReducer extends Reducer<KeyValue, Text, Text, Text> {
    ...
    protected void reduce(KeyValue key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
        ...
        if (iter.hasNext()) {
            ...
            Text outText = new Text(formatString(AIRPORT_NAME_LENGTH, airportName)
                    + "Average delay time: " + formatString(DELAY_LENGTH, String.valueOf(average))
                    + "Maximum delay time: " + formatString(DELAY_LENGTH, String.valueOf(max))
                    + "Minimum delay time: " + formatString(DELAY_LENGTH, String.valueOf(min)));
